...
def dump_optimise(schema: dict[str, Any]) -> Iterable[str]:
    """Format configured optimisation parameters and acquisition settings."""
    ...
    yield " - Reference normalisation: " + str(
        optimise.get("reference_normalisation", "none")
    )
    yield " - Reference resample interval: " + str(
...
def summarise(schema: dict[str, Any]) -> str:
    ...
    section_title = (
        "Optimise settings" if execution_mode == "optimise" else "Scan settings"
    )
    result += section_title + "\n"
    result += "=" * len(section_title) + "\n"
    result += "\n"
    lines = dump_optimise(schema) if execution_mode == "optimise" else dump_scan(schema)
    for s in lines:
        result += s + "\n"
    result += "\n"
