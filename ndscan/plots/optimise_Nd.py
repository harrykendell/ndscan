...
class OptimisePlotWidget(SliceableMenuPanesWidget):
    def __init__(self, model: ScanModel):
        ...
        self.convergence_objective_color = QtGui.QColor(0, 0, 0)
    ...
    def _set_convergence_legend(self, entries):
        ...
        for color, text in entries:
            parts.append(
                ...
                "&nbsp;<span style='color: #000000'>{text}</span>".format(
                    color=self._html_color(color), text=html.escape(text)
                )
            )
    ...
    def _html_color(color):
        return QtGui.QColor(color).name(QtGui.QColor.NameFormat.HexRgb)
