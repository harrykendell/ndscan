"""Interpolated views of irregular multidimensional optimisation data."""
...
from .image_2d import (
    ClickableImageItem,
    CrosshairZDataLabel,
    _calc_range_spec,
    _coords_to_indices,
    _num_points_in_range,
)
...
from .plot_widgets import SliceableMenuPanesWidget, add_source_id_label, add_time_slider
from .utils import (
...
def _channel_label(channel, fallback: str) -> str:
...
def _image_pixel_centres(
    ...
    """Return data coordinates for pixel centres under ``ImageItem.setRect``.

    The image rectangle extends half an increment beyond the nominal range. If the
    increment does not divide the range exactly, its pixels are consequently not
    centred on ``linspace(minimum, maximum)``.
    """
    minimum, maximum, increment = range_spec
    lower_edge = minimum - increment / 2
...
class _DelaunayInterpolationLayer:
    ...
    def interpolate(self, fallback_data: np.ndarray) -> np.ndarray:
        ...
        x_coords = _image_pixel_centres(self.x_range, fallback_data.shape[0])
        y_coords = _image_pixel_centres(self.y_range, fallback_data.shape[1])
...
class _ImagePlot:
    ...
    def update(self, averaging_enabled):
        ...
        # A history-slider rewind shortens the arrays without necessarily changing
        # their common prefix. Rebuild all accumulated averages and interpolation
        # state so points after the cutoff do not remain in the surface.
...
class OptimisePlotWidget(SliceableMenuPanesWidget):
    def __init__(self, model: ScanModel):
        ...
        self.convergence_curve_colors = []
        self.convergence_objective_curve = None
        self.convergence_objective_color = pyqtgraph.getConfigOption("foreground")
        ...
        self.convergence_plot_item.setLabel("left", "Best values", units="normalised")
        self.convergence_plot_item.getAxis("left").autoSIPrefix = False
    ...
    def _update_parameter_convergence(self, points):
        ...
        while len(self.convergence_curves) < len(self.model.axes):
            ...
            self.convergence_curve_colors.append(color)

        if self.convergence_objective_curve is None:
            self.convergence_objective_curve = pyqtgraph.PlotDataItem(
                pen=pyqtgraph.mkPen(self.convergence_objective_color, width=2.0),
            )
            self.convergence_plot_item.addItem(self.convergence_objective_curve)
        ...
        legend_entries = [
            (
                ...
                f"Objective: {self._format_channel_value(channel, z_data[best_idx])}",
            )
        ]
        for axis_idx, color in enumerate(self.convergence_curve_colors):
            schema = self.model.axes[axis_idx]
            param = schema["param"]
            name = param["fqn"].split(".")[-1]
    ...
    def _format_tracking_value(self, schema, value):
        ...
        try:
            return format_label_value(
                float(value), data_to_display_scale, (value, value), unit_suffix
            )
        except (TypeError, ValueError):
            return str(value)

    def _set_convergence_legend(self, entries):
        parts = []
        text_color = self._html_color(pyqtgraph.getConfigOption("foreground"))
        for color, text in entries:
            parts.append(
                "<span style='color: {color}'>&#9473;&#9473;&#9473;</span>"
                "&nbsp;<span style='color: {text_color}'>{text}</span>".format(
                    color=self._html_color(color),
                    text_color=text_color,
                    text=html.escape(text),
                )
            )
        self.convergence_plot_item.setTitle("  ".join(parts), size="10pt")

    def _format_channel_value(self, channel, value):
        unit_suffix, data_to_display_scale = get_axis_scaling_info(channel)
        limits = channel.get("min"), channel.get("max")
        if limits[0] is None or limits[1] is None:
            limits = value, value
        try:
            return format_label_value(
                float(value), data_to_display_scale, limits, unit_suffix
            )
        except (TypeError, ValueError):
            return str(value)

    @staticmethod
    def _html_color(color):
        # pyqtgraph accepts shorthand colours such as ``"w"`` and ``"d"`` in
        # its theme configuration; QColor does not parse those consistently.
        return pyqtgraph.mkColor(color).name(QtGui.QColor.NameFormat.HexRgb)
    ...
    def _normalise_objective_values(self, values):
    ...
    def time_cutoff_changed(self, cutoff: int):
        if cutoff == -1:
            return
        highlighted = self.selected_point_model.get_source_index()
        if highlighted is not None and highlighted > cutoff:
            self._highlight_point_at_index(None)
