...
class KernelPersistentOptimiseErrorFragment(ExpFragment):
    def build_fragment(self):
...
KernelPersistentOptimiseErrorScan = make_fragment_scan_exp(
    KernelPersistentOptimiseErrorFragment, max_transitory_error_retries=1
)
...
class KernelOptimizeCase(KernelEmulatorCase):
    def test_persistent_transitory_error_can_be_scored_as_bad_point(self):
        exp = self.create(KernelPersistentOptimiseErrorScan)
        exp.args._params["execution_mode"] = "optimise"
        exp.args._params["optimise"] = {
            "parameters": [
                {
                    "fqn": (
                        "test_experiment_kernel.KernelPersistentOptimiseErrorFragment.x"
                    ),
                    "path": "*",
                    "min": -1.0,
                    "max": 1.0,
        ...
        self.assertTrue(self.dataset_db.get("ndscan.rid_0.completed"))
        self.assertEqual(
            self.dataset_db.get("ndscan.rid_0.optimizer.termination_reason"),
            "max_evals_reached",
        )

    def test_kernel_fragment_optimise_smoke(self):
        ...
        self.assertTrue(self.dataset_db.get("ndscan.rid_0.completed"))
        self.assertEqual(self.dataset_db.get("ndscan.rid_0.execution_mode"), "optimise")
        self.assertEqual(len(self.dataset_db.get("ndscan.rid_0.points.axis_0")), 1)
        self.assertEqual(len(self.dataset_db.get("ndscan.rid_0.points.axis_1")), 1)
        self.assertEqual(
            len(self.dataset_db.get("ndscan.rid_0.points.channel_objective")), 1
        )
